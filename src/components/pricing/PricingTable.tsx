"use client";

import { useState } from "react";
import Link from "next/link";
import { motion } from "framer-motion";
import { Check, Sparkles } from "lucide-react";
import { Reveal } from "@/components/fx/Reveal";
import { pricingTiers } from "@/content/pricing";
import { cn } from "@/lib/utils";

export function PricingTable() {
  const [yearly, setYearly] = useState(false);

  return (
    <div>
      {/* billing toggle */}
      <div className="mb-12 flex items-center justify-center gap-4">
        <span className={cn("text-sm font-medium", !yearly ? "text-foreground" : "text-foreground/50")}>
          Monthly
        </span>
        <button
          type="button"
          role="switch"
          aria-checked={yearly}
          onClick={() => setYearly((v) => !v)}
          className={cn(
            "relative h-7 w-14 rounded-full border border-foreground/10 transition-colors",
            yearly ? "bg-brand-gradient" : "bg-foreground/10"
          )}
        >
          <motion.span
            layout
            transition={{ type: "spring", stiffness: 500, damping: 32 }}
            className={cn(
              "absolute top-0.5 h-[22px] w-[22px] rounded-full bg-white shadow",
              yearly ? "right-0.5" : "left-0.5"
            )}
          />
        </button>
        <span className={cn("flex items-center gap-2 text-sm font-medium", yearly ? "text-foreground" : "text-foreground/50")}>
          Yearly
          <span className="rounded-full bg-gold/15 px-2 py-0.5 text-xs font-semibold text-[rgb(var(--gold-soft))]">
            Save 20%
          </span>
        </span>
      </div>

      <div className="grid items-start gap-6 lg:grid-cols-3">
        {pricingTiers.map((tier, i) => {
          const price = yearly ? tier.yearly : tier.monthly;
          return (
            <Reveal key={tier.name} delay={i * 0.08}>
              <div
                className={cn(
                  "relative flex h-full flex-col rounded-4xl border p-7 transition-all duration-500",
                  tier.highlighted
                    ? "gradient-border border-transparent bg-card shadow-card lg:-mt-4 lg:mb-4"
                    : "border-foreground/10 bg-card/50 hover:border-gold/30"
                )}
              >
                {tier.highlighted && (
                  <span className="absolute -top-3 left-1/2 inline-flex -translate-x-1/2 items-center gap-1.5 rounded-full bg-brand-gradient px-3 py-1 text-xs font-bold text-ink-950">
                    <Sparkles className="h-3.5 w-3.5" /> Most popular
                  </span>
                )}

                <h3 className="font-display text-xl font-bold">{tier.name}</h3>
                <p className="mt-1.5 min-h-[40px] text-sm text-foreground/60">{tier.tagline}</p>

                <div className="mt-5 flex items-end gap-1">
                  <span className="font-display text-5xl font-extrabold">${price}</span>
                  <span className="mb-1.5 text-sm text-foreground/50">{price === 0 ? "forever" : "/ month"}</span>
                </div>
                {yearly && price > 0 && (
                  <p className="mt-1 text-xs text-foreground/45">billed annually</p>
                )}

                <Link
                  href={tier.cta.href}
                  className={cn(
                    "mt-6 inline-flex h-12 items-center justify-center rounded-full text-sm font-semibold transition-transform hover:-translate-y-0.5",
                    tier.highlighted
                      ? "bg-brand-gradient text-ink-950"
                      : "border border-gold/40 text-foreground hover:bg-gold/10"
                  )}
                >
                  {tier.cta.label}
                </Link>

                <ul className="mt-7 space-y-3">
                  {tier.features.map((f) => (
                    <li key={f} className="flex items-start gap-2.5 text-sm text-foreground/75">
                      <span className="mt-0.5 grid h-4 w-4 shrink-0 place-items-center rounded-full bg-gold/15">
                        <Check className="h-3 w-3 text-[rgb(var(--gold-soft))]" />
                      </span>
                      {f}
                    </li>
                  ))}
                </ul>
              </div>
            </Reveal>
          );
        })}
      </div>
    </div>
  );
}
